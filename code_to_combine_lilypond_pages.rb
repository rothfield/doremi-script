...
class LilypondServer < Sinatra::Application
		...
		set :doremi2htmldoc, "/usr/local/bin/doremi2htmldoc"
		...
		post '/ly_to_jpg' do
				ly=params["ly"]
				digest=Digest::MD5.hexdigest(ly)
				fname=digest
				digest
				comp=settings.comp
				dir=File.join('public','compositions')
				return "no ly parameter" if !params["ly"]
				fp= "#{comp}/#{fname}"
				if not (File.exist? "#{fp}.jpg") 
				File.open("#{fp}.ly", 'w') {|f| f.write(ly) }
		...
		post '/lilypond_to_jpg' do
				...
				doremi2htmldoc_result= `cat #{fp}.doremi_script.txt | #{settings.doremi2htmldoc} > #{fp}.html 2>&1`  
