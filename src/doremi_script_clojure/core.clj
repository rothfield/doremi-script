(ns doremi_script_clojure.core
  ...
  (:require	
    ...
    [doremi_script_clojure.new_core :refer [experiment test-all2]]
    [doremi_script_clojure.to_lilypond :refer [to-lilypond]]
    [clojure.java.io :as io :refer [file  input-stream resource]]
...
(defn usage[]
  (println "Usage: pipe std in as follows: \"SRG\" |  java -jar doremi-script-standalone.jar > my.ly to produce lilypond output. Or --json to produce doremi-script json format. Or --ly to produce lilypond output. Or --test-all") 
  )


(defn -main[& args]
  ;; (println (file-seq (-> "fixtures" io/resource io/file)))
